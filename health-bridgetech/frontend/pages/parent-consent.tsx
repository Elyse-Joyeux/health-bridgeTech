...
export function ParentConsentPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    if (!parent || !parentName.trim())
      return setError('Please enter your guardian\u2019s name and email.');
    if (parent === userEmail) {
      return setError(
        "Parent's email cannot be the same as your account email."
      );
    }
  ...
  const resend = async () => {
    setResending(true);
    try {
      const res = await api<{ verifyUrl: string }>(
        '/api/auth/parent-consent/resend',
        {
          method: 'POST',
        }
      );
      ...
      <div className={styles.shell}>
        ...
        <div className={styles.card}>
          ...
          <p className={styles.subtitle}>
            ...
            <strong style={{ color: 'var(--hb-primary)' }}>
              {status.parentEmail}
            </strong>
            . Once {status.parentName || 'your guardian'} clicks it, your
            sanctuary will unlock automatically.
          </p>

          <div
            style={{
              background: 'var(--hb-mint-pale)',
              border: '1px solid var(--hb-mint)',
              borderRadius: 14,
              padding: 16,
              marginBottom: 16,
              display: 'flex',
  ...
  return (
    <div className={styles.shell}>
      ...
      <div className={styles.card}>
        ...
        <p className={styles.subtitle}>
          Because {user?.displayName || 'you'} are under 18, we&rsquo;ll email
          your parent or guardian to confirm before unlocking biometric tracking
          and the AI Companion.
        </p>

        <div
          className={styles.minorBanner}
          style={{ alignItems: 'flex-start', marginBottom: 18 }}
        >
          <ShieldIcon size={18} />
          <div>
            <strong>How it works:</strong> we&rsquo;ll send a one-time
            verification link to the parent email below. Once they click it,
            your account is automatically activated. Use a real guardian inbox
            &mdash; not your own.
          </div>
        </div>
        ...
        <form onSubmit={onSubmit}>
          ...
          <div className={styles.field}>
            ...
            {parentEmail &&
              parentEmail.toLowerCase() === user?.email.toLowerCase() && (
                <div className={styles.matchError}>
                  That&rsquo;s your own email. Please enter a guardian&rsquo;s
                  address.
                </div>
              )}
          </div>
          ...
          <label className={styles.checkbox}>
            ...
            <div>
              ...
              <span>
                I understand they will receive a verification email and must
                approve before my account is activated. The email address above
                belongs to a real adult guardian who can speak on my behalf.
              </span>
            </div>
          </label>
