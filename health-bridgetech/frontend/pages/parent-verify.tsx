...
export function ParentVerifyPage() {
  ...
  useEffect(() => {
    ...
    api<ConsentDetails>(
      `/api/auth/parent-consent/details?token=${encodeURIComponent(token)}`,
      {
        auth: false,
      }
    )
      .then((d: ConsentDetails) => {
        setDetails(d);
      ...
      <div className={styles.card}>
        ...
        {state === 'loading' && (
          <>
            ...
            <p className={styles.subtitle}>
              Hang tight while we look up the details.
            </p>
          </>
        )}
        ...
        {state === 'review' && details && (
          <>
            <h1 className={styles.title}>
              Approve {details.childDisplayName}&rsquo;s account
            </h1>
            <p className={styles.subtitle}>
              Please review the request and confirm your consent so{' '}
              {details.childDisplayName} can start using Health BridgeTech.
            </p>
            ...
            >
              <ShieldIcon size={18} />
              <div>
                <strong>What you&rsquo;re consenting to:</strong>{' '}
                {details.childDisplayName}
                &rsquo;s use of Health BridgeTech, including biometric
                monitoring from connected wearables, AI Companion conversations,
                care-team messaging, and community circles. All data is
                HIPAA-compliant and you can revoke consent at any time by
            ...
            <form onSubmit={submit}>
              <label className={styles.checkbox}>
                ...
                <div>
                  <strong>
                    I confirm I&rsquo;m {details.childDisplayName}&rsquo;s
                    parent or legal guardian
                  </strong>
                  <span>
                    I consent to {details.childDisplayName}&rsquo;s use of
                    Health BridgeTech as described above, and I understand
        ...
        {state === 'success' && (
          <>
            ...
            <p className={styles.subtitle}>
              Thank you. {details?.childDisplayName || 'Your child'}&rsquo;s
              account is now active. They can return to Health BridgeTech and
              continue setting up their sanctuary.
            </p>
            <Link
              to="/"
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    ...
    >
      <span style={{ color: 'var(--hb-text-muted)', fontSize: 12.5 }}>
        {label}
      </span>
      <span style={{ fontWeight: 600, fontSize: 13, textAlign: 'right' }}>
        {value}
      </span>
    </div>
  );
